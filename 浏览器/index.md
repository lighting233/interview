...
### 3.渲染进程都包括哪些线程？
1. **主线程（GUI 线程）：**
负责处理页面的渲染、JavaScript 执行和 DOM 操作。控制浏览器上的按钮及输入框，接收事件输入
...
7. **storage thread**： 控制文件等的访问；cookie、localStorage、sessionStorage、IndexedDB 等。
8. **Service Workers**
...
1. 首先接受到事件信息的是Browser Process
2. Browser Process只知道事件发生的类型和发生的位置
3. Browser Process接受到事件后，随后便把事件的信息传递给了渲染进程的合成线程。合成器线程会标记页面中绑定有事件处理器的区域为非快速滚动区域(non-fast scrollable region)，如果事件发生在这些存在标注的区域，合成器线程会把事件信息发送给主线程，等待主线程进行事件处理，如果事件不是发生在这些区域，合成器线程则会直接合成新的帧而不用等到主线程的响应。
...
## 六、chrome 浏览器是每个渲染进程里有一个网络线程，浏览器单独有一个网络进程？
，**Chrome 浏览器的架构采用了多进程模型**，其中包括一个独立的**网络进程**和**每个渲染进程中的网络线程**。下面是它的具体工作方式：

### 1. **网络进程（独立）**
   - **Chrome 浏览器有一个单独的网络进程**，负责处理所有网络请求，如 HTTP、HTTPS、DNS、WebSocket 等。网络进程在整个浏览器中是共享的，无论页面有多少个标签或窗口，都会通过这个独立的网络进程处理网络通信。
   - **网络进程的职责**：
     - 发送和接收网络请求。
     - 处理 DNS 解析、TCP/IP 连接、SSL/TLS 加密等。
     - 缓存资源，减少重复的网络请求。
   
### 2. **渲染进程中的网络线程**
   - **每个页面（或标签页）都有自己的渲染进程**，负责处理页面的内容渲染、JavaScript 执行、CSS 样式计算、DOM 操作等。
   - 每个渲染进程内部有一个专用的**网络线程**，负责与网络进程进行通信。这些网络线程不会直接发起网络请求，而是通过网络进程代理请求并返回响应。
   - 渲染进程的网络线程负责：
     - 向网络进程发起资源请求（如 HTML、CSS、JS、图片等）。
     - 接收网络进程返回的资源，并将其传递给渲染进程的其他部分，如 JavaScript 引擎或布局引擎。

### 3. **工作机制**：
   - **网络进程**管理所有的网络请求，无论这些请求来自哪个标签页或窗口，它都会统一处理。这有助于优化资源共享和网络性能，如 HTTP/2 多路复用、DNS 缓存等。
   - **渲染进程中的网络线程**通过 IPC（进程间通信）与网络进程交互，发起资源加载请求。网络进程将资源数据返回给相应的渲染进程中的网络线程，之后由渲染线程负责将这些资源解析并应用到页面中。
   - 这种模型的好处是即使某个渲染进程崩溃，网络进程仍然可以继续运行，不影响其他页面的加载和通信。

### 图示化结构：
- **浏览器进程**：管理浏览器界面、用户交互（如地址栏、书签等）。
- **网络进程**：单独的进程，负责所有网络请求的处理。
- **渲染进程**（每个页面有独立的进程）：包含一个网络线程与网络进程通信。
  
### 总结：
- **网络进程**是浏览器中独立的进程，负责所有网络请求和通信。
- **渲染进程中的网络线程**负责发起请求并通过网络进程获取资源。
- 这种设计提高了浏览器的安全性、稳定性和性能。